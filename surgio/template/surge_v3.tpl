#!MANAGED-CONFIG {{ downloadUrl }} strict=false
...
[Proxy Group]
🚀 Proxy = select, 1️⃣ ssrpass.com, 2️⃣ pornsshub.com, 3️⃣ custom
1️⃣ ssrpass.com = select, {{ getNodeNames(nodeList, customFilters.ssrpass)}}
2️⃣ pornsshub.com = select, policy-path={{ getDownloadUrl('SurgeV3.conf?format=surge-policy&filter=customFilters.pornsshub')}}
3️⃣ custom = select, policy-path={{ getDownloadUrl('SurgeV3.conf?format=surge-policy&filter=customFilters.custom')}}
🎬 Netflix = select, 🚀 Proxy, {{ getNodeNames(nodeList, customFilters.AmericanHighRate) }}
🎵 Spotify = select, 🚀 Proxy, {{ getNodeNames(nodeList, customFilters.AmericanHighRate) }}
...
🖥 Microsoft = select, DIRECT, 🚀 Proxy, 3️⃣ custom, 🇺🇸 US, 🇯🇵 JP
☁️ OneDrive = select, DIRECT,  🚀 Proxy, 3️⃣ custom, 🇺🇸 US, 🇯🇵 JP, 🇭🇰 HK
...
🇭🇰 HK = load-balance, {{ getNodeNames(nodeList, customFilters.HongKongHighRate) }}, evaluate-before-use = true
🇯🇵 JP = load-balance, {{ getNodeNames(nodeList, customFilters.JapanHighRate) }}, evaluate-before-use = true 
🇸🇬 SG = load-balance, {{ getNodeNames(nodeList, customFilters.SingaporeHighRate) }}, evaluate-before-use = true 
🇹🇼 TW = url-test, {{ getNodeNames(nodeList, customFilters.TaiwanHighRate) }}, url = {{ proxyTestUrl }}, interval = 300, tolerance = 100, timeout=5

[Rule]

DOMAIN, mirror.isoc.org.il, REJECT
